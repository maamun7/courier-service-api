...
@section('content')
    ...
    <div class="m-portlet">
            ...
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    <h3 class="m-portlet__head-text">
                        ...
                        <a href="{!! route('admin.import.files') !!}" class="btn btn-sm m-btn--pill    btn-outline-success">
                            <span class="la la-file-excel-o"></span>&nbsp; Upload CSV
                        </a>
                    </h3>
                </div>
            </div>
        ...
        <?php if (Request::is('admin/amendment-delivery')) {
            $sortingUrl = route('admin.delivery.amendment-hub.store');
        }else{$sortingUrl = route('admin.delivery.hub.store');} ?>
    ...
    <div class="modal fade" id="product-payment-received-Modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                <form id="" method="post" action="{{route('admin.delivery.hub.store.payment.received')}}" class="product_details_form">
                        {{csrf_field()}}
...
{{--                        </div>--}}
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            var table = $('#Deliverys-datatable').DataTable({
                ...
                dom : 'l<"#date-filter"><"#sorting-filter"><"#hub-filter"><"#unsolved-filter">frtip',
                ajax: {
                    ...
                    data: function( d ) {
                        ...
                        d.query_string = "{{ !empty($_GET['query_string']) ? $_GET['query_string'] : ''}}";
                        d.status = "{{ !empty($_GET['status']) ? $_GET['status'] : ''}}";
                    }
                },
                columns: [
                    {
                    ...
                    {
                        data: 'flag_status_modified_at',
                        name: 'flag_status_modified_at',
                        searchable: false,
                        render: function ( data, type, row ) {
                            var flag_status_modified_at = row.flag_status_modified_at;
                            return '<b style="color: '+row.color_code+';">'+flag_status_modified_at+'</b>';
                        }
                ...
                "footerCallback": function ( row, data, start, end, display ) {
                    ...
                    var total_collected_amount = api.ajax.json().totalCollected;
                    var total_received_amount = api.ajax.json().totalReceived;

                    // Update footer
                    $( api.column( 11 ).footer() ).html(
                        'Page Total: ' + Math.round(pageTotalCollectedAmount) +' ( Total: '+ total_collected_amount + ' )'
                    );

                    $( api.column( 12 ).footer() ).html(
                        'Page Total: ' + Math.round(pageTotalReceivedAmount) +' ( Total: '+ total_received_amount + ' )'
                    );
            ...
            $('#unsolved-filter').addClass('col-lg-1 col-md-1 col-sm-1');
            $('#Deliverys-datatable_filter').addClass('col-lg-3 col-md-3 col-sm-3');
            $('#Deliverys-datatable_filter .form-control').addClass('search_filter');
            ...
            var unsolved_html =
                '<span href="" style="cursor: pointer" class="btn btn-outline-success" id="unsolved-items">Unsolved Items</span>';

            $('#unsolved-filter').append(unsolved_html);
            $('#unsolved-items').click(function() {
                var unsolve = 'clicked';
                table.columns(15).search(unsolve).draw();
            });
            ...
            $("#barcode_scanner").change(function () {
                ...
                if(parcel[0] == 'PARCELBD' && parcel[1].length == 8)
                {
                    ...
                    var resp = table.columns(0).search(consignment).draw();
                    if (resp == '')
                    {
    ...
    <script type="text/javascript">
        ...
        $(document).on('click','.export-file',function(e)
        {
            ...
            var status =$('#sorting_role').val();
            var hub_id =$('#sorting_hub').val();
            var search_keyword =$('.search_filter').val();
            ...
            $.ajax({
                ...
                data: {"_token": "{{ csrf_token() }}", "export_type": exportType, "start_date": startDate, "end_date": endDate, "status": status, "hub_id": hub_id, "search_keyword": search_keyword},
                cache: false,
                beforeSend: function(){
                ...
                ({
                    ...
                    success: function(res)
                    {
                        ...
                        $.each(res.response, function (key, val) {
                            ...
                            content += '<input type="hidden" class="form-control" value="' + val['status'] + '" name="consignment_current_status_'+val['deli_id']+'">';
